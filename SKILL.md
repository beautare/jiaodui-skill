...
> 更新日期：2026-09-04。开源仓库：https://github.com/beautare/jiaodui-skill（MIT）。

## 快速使用（CLI 脚本，无需安装 Python 包）

三系统脚本位于 `scripts/`（依赖：`curl` + `python3`，仅用于组装/解析 JSON）：
...
服务端直连引擎，无需装任何东西。Streamable HTTP，Bearer key 鉴权：
...
- **Limits**: HTTP body ≤ 1MB；单次输入 ≤ 40,000 字；每个 key 限流 2 QPS（超限 429）；每个账户最多 5 个有效 key
